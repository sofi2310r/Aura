  ...
  async iniciarSesion(): Promise<void> {
    ...
    try {
      ...
      Swal.fire({
        ...
        const userRole = (user.rol ?? (user.role as UserRole) ?? 'paciente');
        this.router.navigate([this.resolveRoute(userRole)]);
      });
    } catch (error: any) {
      this.ngZone.run(() => {
        this.cargandoLogin = false;
      ...
      } else if (errorCode === 'TOKEN_NO_RECIBIDO') {
        mensaje = 'No fue posible iniciar sesión en este momento. Intenta nuevamente.';
      }
